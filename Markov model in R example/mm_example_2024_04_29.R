...
# @knitr stm_1_params
# this lists everything contained in the global environment 
# good to enforce that we are starting from scratch
rm(list = ls())

# no need to install any packages, everything is in base R
# basic information:
...
m_P <- matrix(data = NA, # fill with empty values
              nrow = n_states, #will have this many rows
              ncol = n_states, # will have this many columns 
              dimnames = list(v_n, v_n)) #names of columns and rows will be based on what we previously created 

m_P

### From Healthy
m_P["H", ]  <- c(1 - (p_HS1 + p_HD), p_HS1, 0, p_HD) # just creating the first row of the matrix
...
m_TR <- matrix(data = NA,
               nrow = n_t,  #number of cycles
               ncol = n_states, #number of health states individuals can reside in
               dimnames = list(1:n_t, v_n)) #columns names of states, and the rows are cycles

head(m_TR)      # The head() function enables you to view the top of a table rather than the full matrix

# initialize Markov trace
m_TR[1, ] <- c("H" = 1, "S1" = 0, "S2" = 0, "D" = 0) # everyone is starting healthy
...
for (t in 2:n_t){ # throughout the number of cycles
  ...
  m_TR[t, ] <- m_TR[t - 1, ] %*% m_P
  ## sometime you see   m_TR[t+1, ] <- m_TR[t, ] %*% m_P
